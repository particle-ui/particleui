...
export function Marquee() {
  return (
    <div className="relative w-full overflow-hidden border-y border-white/[0.05] py-4">
      {/* Fade edges */}
      <div className="pointer-events-none absolute left-0 top-0 bottom-0 w-24 z-10 bg-gradient-to-r from-[#030303] to-transparent" />
      <div className="pointer-events-none absolute right-0 top-0 bottom-0 w-24 z-10 bg-gradient-to-l from-[#030303] to-transparent" />
        ...
        >
          <span
            className={cn(
              ...
              item.tier === "pro" ? "bg-[#00d4ff]" : "bg-[#333]"
            )}
          />
          <span className="font-mono text-xs text-[#666]">{item.name}</span>
          {item.tier === "pro" && (
            <span className="text-[9px] font-semibold uppercase tracking-wider text-[#00d4ff]/60">
              pro
            </span>
          )}
        </div>
